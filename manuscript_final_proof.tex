...
\address{%
$^{1}$ \quad Bureau de Recherches G\'eologiques et Mini\`eres (BRGM)\hl{,} %MDPI: We changed the dot to a comma. Please confirm. The following highlight is the same
 3 Avenue Claude-Guillemin, 45060 Orl\'eans, France; polina.lemenkova@tech-orleans.fr; Tel.: +33-063-314-8601

$^{2}$ \quad Le Studium, Institut d'Etudes Avanc\'ees du Val de Loire\hl{,} 1 rue Dupanloup, 45000 Orl\'eans, France
...
$^{4}$ \quad La Technopole d'Orl\'eans\hl{,} 1 avenue du Champ de Mars, 45100 Orl\'eans, France
}
...
\PACS{\hl{92.10.Vv;} %MDPI: Please confirm if 'PACS' section is unnecessary and can be removed.
 95.75.Qr; 42.68.Wt; 93.30.Ge}
\MSC{\hl{62H30;} %MDPI: Please confirm if 'MSC' section is unnecessary and can be removed.
 68U35; 62M30; 68T05}
\JEL{\hl{Q24;} %MDPI: Please confirm if 'JEL' section is unnecessary and can be removed..
 Q54; R14; Q57; Q01}
...
The French coastline stretches over approximately \hl{5853} %MDPI: Commas are only used for numbers with five or more digits. We have removed them in four-digit numbers. Please confirm.
...
\hl{The} %MDPI: We moved the paragraph here to avoid big blank space. Please confirm
...
\caption{\hl{Detailed} %MDPI: We moved the figure after the first citation. Please confirm.
...
\begin{tabularx}{\fulllength}{ p{1.5cm}  p{2.0cm}  p{3.5cm}  p{3.5cm}  p{4.4cm} c }
 \toprule
\textbf{Region} & \textbf{Climate} & \textbf{Coastal Type} & \textbf{Main Land Uses} & \textbf{Main Pressures} & \textbf{\hl{Citation} %MDPI: Please confirm if there is any content missing in this column.
} \\
\midrule
...
\includegraphics[width=15.5cm]{Figure_climate.png}
\end{adjustwidth}
\caption{{\hl{Annual} %MDPI: Please change the hyphen (-) between numbers into an en dash (−, “U+2013”), e.g., “2019-2022” should be “2019–2022” and “0-9” should be “0–9”.
...
\begin{enumerate}[leftmargin=*,labelsep=5mm]
\item \textit{\hl{Data import and reprojection}%MDPI: Please confirm if the italics are necessary; if not, please remove them. The following highlights are the same.
}: All Sentinel-2 tiles were imported into GRASS GIS using \texttt{\hl{r.in.gdal}%MDPI: Please confirm if the special font needs to be retained. Please check all the special fonts in the whole tex
...
\item \textit{\hl{Atmospheric correction verification}}: Level-2A BOA reflectance values were verified for consistency across tiles using inter-tile comparison of stable reference targets (water bodies, dense forest, bare rock). Residual radiometric inconsistencies were corrected through empirical line normalization using \texttt{r.mapcalc}.

\item \textit{\hl{Cloud masking}}: Scene classification layer (SCL) bands provided with Level-2A products were used to generate cloud masks. Pixels classified as cloud (classes 8--10) and cloud shadow (class 3) were excluded using \texttt{r.mapcalc}. Remaining thin cirrus artifacts were identified through visual inspection and masked manually.

\item \textit{\hl{Mosaicking}}: Adjacent tiles within each coastal region were mosaicked using \texttt{r.patch} after reprojection alignment. Mosaic edge effects were minimized by applying a 100-pixel feathering zone along tile boundaries.

\item \textit{\hl{Temporal compositing}}: Summer composites for 2015 and 2025 were generated by applying a median temporal aggregation to the available cloud-free scenes using \texttt{r.series} with the median statistic. Median compositing effectively suppresses residual cloud and shadow contamination while preserving spectral fidelity.

\item \textit{\hl{Vegetation indices}}: Spectral indices were computed to augment the band stack for classification. The normalized difference vegetation index (NDVI) and normalized difference water index (NDWI) were computed using \texttt{i.vi} and \texttt{r.mapcalc}, respectively, following standard formulations.

\item \textit{\hl{Temporal stack preparation}}: Final multispectral composite rasters were organized into GRASS GIS temporal databases using \texttt{t.rast.series} to facilitate time-series analysis and change detection workflows.
\end{enumerate}
...
\end{adjustwidth}
\caption{{\hl{Detailed} %MDPI: 1. Please confirm whether an explanation of the colors in 1--6 needs to be added to the figure caption. 2. Please confirm whether the unclear content in the map affects scientific understanding and if it does, please revise it. 3. To avoid large blank spaces on the page, we moved Figure 4 to Section 3.4. Please confirm
...
\begin{figure}[H]
%\centering
\includegraphics[width=11.8cm]{Figure_workflow.png}
\caption{{\hl{Workflow} %MDPI: 1.  To avoid large blank spaces on the page, we moved Figure 4 to Section 3.4. Please confirm. 2. Please change the hyphen (-) between numbers into an en dash (−, “U+2013”), e.g., “2019-2022” should be “2019–2022” and “0-9” should be “0–9”.
...
\begin{table}[H]
\caption{\hl{Land} %MDPI: Please cite this table in the text and ensure that the first citation of each table appears in numerical order.
...
\begin{table}[H]
\caption{\hl{Classification} %MDPI: Please confirm the alignment change.
...
\caption{\hl{GRASS} %MDPI: We moved the figure after the first citation. Please confirm
...
\includegraphics[width=13.8cm]{Figure_05.png}
	\caption{\hl{Land} %MDPI: Please change the hyphen (-) into a minus sign (−, “U+2212”) in the figure, e.g., “-1” should be “−1”.
...
\begin{table}[H]
\caption{\hl{Regional} %MDPI: Please confirm whether the alignment of the table needs to be maintained.
...
\caption{\hl{Spatial } %MDPI: Please confirm whether the unclear content (the superscript number of the unit 'km') in this figure affects scientific understanding and if it does, please revise it.
...
\begin{table}[H]
\caption{\hl{Dominant} %MDPI: Please confirm whether the alignment of the table needs to be maintained.
...
\caption{{\hl{Conceptual} %MDPI: Please change the hyphen (-) between numbers into an en dash (−, “U+2013”), e.g., “2019-2022” should be “2019–2022” and “0-9” should be “0–9”.
...
\funding{\hl{This} %MDPI: The Author Contributions section was removed, as it is unnecessary when there is only one author. Please confirm.
...
% ---- Script 1: r.in.gdal -------------------------------------------------
\subsection{\highlighting{Script 1:} %MDPI: We changed the command '\subsubsection' without section number to subsection and added section number. Please confirm. The following highlights are the same
...
% ---- Script 2: i.sentinel -------------------------------------------------
\subsection{\highlighting{Script 2: Sentinel-2 Band Management with \texttt{i.sentinel}}}
...
% ---- Script 3: r.mapcalc --------------------------------------------------
\subsection{\highlighting{Script 3: Cloud Masking and Radiometric Normalization with \texttt{r.mapcalc}}}
...
% ---- Script 4: r.series ---------------------------------------------------
\subsection{\highlighting{Script 4: Median Temporal Compositing with \texttt{r.series}}}
...
\subsection{\highlighting{Script 5: Temporal Raster Database Management with \texttt{t.rast.series}}}
...
% ---- Script 6: i.vi -------------------------------------------------------
\subsection{\highlighting{Script 6: Spectral Index Computation with \texttt{i.vi}}}
...
% ---- Script 7: r.reclass --------------------------------------------------
\subsection{\highlighting{Script 7: Post-Classification Reclassification with \texttt{r.reclass}}}
...
\newblock Chiffres cl{\'e}s de la mer et du littoral---{\'E}dition 2024,
  2024.  \hl{Available online:} %MDPI: Please add the url.
...
\newblock Technical Report, Copernicus Land Monitoring Service;  \hl{European Environment Agency: Copenhagen, Denmark,} %MDPI: We added the name of the publisher and their location. Please confirm. The following highlights are the same.
 2020;
...
\newblock Do second home owners only play a secondary role in coastal
  ...
  Routledge Handbook of Second Home Tourism and Mobilities}; Routledge: \hl{Abingdon, UK,} %MDPI: We added the location of the publisher. Please confirm. The following highlights are the same.
  2018;
  pp. 233--244.
...
\newblock \textit{Living with Coastal Erosion in Europe: Sediment and Space for
  ...
  studies; \hl{Eurosion: Hague, The Netherlands,} 2004.
\newblock 
...
\newblock {\em Climate Change 2022: Impacts, Adaptation and Vulnerability}; \hl{IPCC: Geneva, Switzerland,} {
  2022}.
...
\newblock {\em Image and Signal Processing for Remote Sensing XXIII}; \hl{SPIE: Bellingham, WA, USA,} {2017}; {Volume 10427}.
...
\newblock {Publications Office of the European Union}: \hl{Luxembourg,} {2016}.
...
{INSEE}.
\newblock\textit{\hl{ Les populations du littoral m\'editerran\'een fran\c{c}ais}%MDPI: We set this as the book title. Please confirm
};  Analyse d\'emographique des espaces littoraux m\'editerran\'eens; \hl{INSEE: Montrouge, France,} 2017.
...
\newblock Technical report, Copernicus Land Monitoring Service;  \hl{European Environment Agency: Copenhagen, Denmark,} 2020;
\newblock Version 2020\_20u1.
